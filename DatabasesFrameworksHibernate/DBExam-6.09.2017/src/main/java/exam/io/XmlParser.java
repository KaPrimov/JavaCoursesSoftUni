...
public class XmlParser implements Serializer {
    ...
    @Autowired
    public XmlParser(FileParser fileParser) throws JAXBException {
    ...
    public <T> T deserialize(Class<T> tClass, String fileName) {
        try {
            ...
            throw new SerializeException("Could not deserialize to class" + tClass, e);
        } catch (IOException ioe) {
            throw new SerializeException("Could not deserialize to class" + tClass + " Unable to read from file " + fileName, ioe);
        }
    }
